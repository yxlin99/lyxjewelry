...
public class OrderDao {
	...
	public void addProduct(Order order) throws SQLException {
		// 1.生成Sql语句
		String sql = "insert into orders(id,money,receiveAddress,receiveName,receivePhone,paystate,orderTime,user_id) values(?,?,?,?,?,0,?,?)";
		...
		runner.update(DataSourceUtils.getConnection(), sql, order.getId(),
				...
				order.getReceiverPhone(), order.getOrdertime(), order.getUser().getId());
	}
	/**
	 * 查找用户所属订单
	 */
		...
		return runner.query(sql, new ResultSetHandler<List<Order>>() {
			public List<Order> handle(ResultSet rs) throws SQLException {
				...
				while (rs.next()) {
					...
					order.setReceiverAddress(rs.getString("receiveAddress"));
					order.setReceiverName(rs.getString("receiveName"));
					order.setReceiverPhone(rs.getString("receivePhone"));
					order.setUser(user);
					orders.add(order);
				}
				return orders;
	...
	public Order findOrderById(String id) throws SQLException {
		String sql = "select * from orders,user where orders.user_id=user.id and orders.user_id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		return runner.query(sql, new ResultSetHandler<Order>() {
			public Order handle(ResultSet rs) throws SQLException {
				...
				while (rs.next()) {
					...
					order.setOrdertime(rs.getString("orders.ordertime"));
					order.setPaystate(rs.getInt("orders.paystate"));
					order.setReceiverAddress(rs.getString("orders.receiveAddress"));
					order.setReceiverName(rs.getString("orders.receiveName"));
					order.setReceiverPhone(rs.getString("orders.receivePhone"));
	...
	public List<Order> findAllOrder() throws SQLException {
		...
		return runner.query(sql, new ResultSetHandler<List<Order>>() {			
			public List<Order> handle(ResultSet rs) throws SQLException {
				...
				while (rs.next()) {
					Order order = new Order();
//					System.out.println(rs.getString("orders.id"));
					order.setId(rs.getString("orders.id"));
					order.setMoney(rs.getDouble("orders.money"));
					order.setOrdertime(rs.getString("orders.ordertime"));
					order.setPaystate(rs.getInt("orders.paystate"));
               ...
               //循环遍历出订单和用户信息
				while (rs.next()) {
					...
					order.setReceiverAddress(rs
							.getString("orders.receiveAddress"));
					order.setReceiverName(rs.getString("orders.receiveName"));
					order.setReceiverPhone(rs.getString("orders.receivePhone"));
					orders.add(order);
					User user = new User();
	...
	public void listAllOrder() throws SQLException {
