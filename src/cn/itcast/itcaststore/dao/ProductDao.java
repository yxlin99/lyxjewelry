...
public class ProductDao {
	...
	public void addProduct(Product p) throws SQLException {
		...
		String sql = "insert into products(id,name,price,category,number,imgurl,material,color,description) values(?,?,?,?,?,?,?,?,?)";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		runner.update(sql, p.getId(), p.getName(), p.getPrice(),
				p.getCategory(), p.getPnum(), p.getImgurl(),p.getMaterial(),p.getColor(), p.getDescription());
	}
	...
	public List<Product> Recommend(User user) throws SQLException {
		int earings=0,necklace=0,set=0,num=0;
		String m;
		...
		{
			m=pr.getCategory();
			if(m.equals("earings")) earings++;
			if(m.equals("necklace")) necklace++;
			if(m.equals("set")) set++;
	...
	public void changeProductNum(Order order) throws SQLException {
		String sql = "update products set number=number-? where id=?";
	...
	public void editProduct(Product p) throws SQLException {
		...
		String sql  = "update products " +
				      "set  name=?,price=?,category=?,number=?,description=? ";
		//判断是否有图片
